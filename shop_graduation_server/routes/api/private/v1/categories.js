...
router.get(
    ...
    function (req, res, next) {
        ...
        if (
            !req.query.sel ||
            (req.query.sel != 'only' && req.query.sel != 'many')
        ) {
...
router.get(
    ...
    function (req, res, next) {
        ...
        if (isNaN(parseInt(req.params.attrId)))
            return res.sendResult(null, 400, '参数ID必须是数字');
        next();
    },
    function (req, res, next) {
...
router.post(
    ...
    function (req, res, next) {
        ...
        if (!req.body.attr_name)
            return res.sendResult(null, 400, '参数名称不能为空');

        if (
            !req.body.attr_sel ||
            (req.body.attr_sel != 'only' && req.body.attr_sel != 'many')
        ) {
            return res.sendResult(
                null,
                400,
                '参数 attr_sel 类型必须为 only 或 many'
            );
        }
        /*
...
router.put(
    ...
    function (req, res, next) {
        ...
        if (!req.body.attr_name || req.body.attr_name == '')
            return res.sendResult(null, 400, '参数名称不能为空');
