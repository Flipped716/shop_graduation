		...
		dao.list("GoodPicModel", {
			...
			_(addNewpics).forEach(function(pic) {
				if (!pic.pics_id && pic.pic) {
					...
					batchFns.push(clipImage(src, path.join(process.cwd(), pic.pics_big), 800,
						800));
					batchFns.push(clipImage(src, path.join(process.cwd(), pic.pics_mid), 400,
						400));
					batchFns.push(clipImage(src, path.join(process.cwd(), pic.pics_sma), 200,
						200));
...
module.exports.getAllGoods = function(params, cb) {
	...
	dao.countByConditions("GoodModel", conditions, function(err, count) {
		...
		conditions["only"] = ["goods_id", "goods_name", "goods_price", "goods_weight", "goods_state",
		...
		dao.list("GoodModel", conditions, function(err, goods) {
			...
			resultDta["goods"] = _.map(goods, function(good) {
				return _.omit(good, "goods_introduce", "is_del", "goods_big_logo",
					"goods_small_logo", "delete_time");
			});
			cb(err, resultDta);
		})
	});
